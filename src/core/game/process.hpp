...
public:
    ...
    // Процесорний час процесу гри (усі потоки), с; -1 — невідомо. Приріст показує, чи гра щось
    // робить (компілює шейдери, рендерить важкий кадр), коли кадрів довго немає, чи справді зависла.
    double   cpu_seconds();
