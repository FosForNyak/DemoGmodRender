...
void App::draw_tab_video() {
    ...
    if (ImGui::BeginCombo("##qpreset", tr("вибрати готовий набір..."))) {
    ...
    ImGui::SetNextItemWidth(ww * 0.55f);
    if (ImGui::BeginCombo("##fps", (s_.fps + tr(" кадрів/с")).c_str())) {
    ...
    ImGui::SetNextItemWidth(ww * 0.55f);
    changed |= ImGui::SliderInt("##blur", &s_.motion_blur, 1, 64, s_.motion_blur <= 1 ? tr("вимкнено") : tr("%d під-кадрів"));
    ...
    label(tr("Швидкість відео"), lw);
    {
        ...
        if (std::abs(s_.speed - 1.0) > 1e-6) {
            ...
            if (ImGui::RadioButton(tr("розтягнути"), s_.speed_audio != "mute")) {
                s_.speed_audio = "stretch";
                changed = true;
            }
            ImGui::SameLine();
            if (ImGui::RadioButton(tr("без звуку"), s_.speed_audio == "mute")) {
                s_.speed_audio = "mute";
                changed = true;
            }
    ...
    ImGui::SeparatorText(tr("Кодек і формат"));
    // ---- Контейнер ----
    ...
    if (qi.param != "profile") {
        ...
        if (use_size) {
            ...
            if (ImGui::InputFloat(tr("МБ##tsize"), &mb, 0, 0, "%.0f")) {
    ...
    for (int d : {8, 10, 12}) {
        ...
        if (ImGui::RadioButton(trf("{} біт##bd{}", d, d).c_str(), s_.bit_depth == d)) {
            s_.bit_depth = d;
            changed = true;
        }
        ImGui::EndDisabled();
    ...
    if (ImGui::TreeNode(tr("Додатково"))) {
        ...
        ImGui::SetNextItemWidth(fs_ * 6);
        changed |= ImGui::InputInt(tr("с##gop"), &s_.gop_seconds);
        s_.gop_seconds = std::clamp(s_.gop_seconds, 0, 60);
        label(tr("Точність кольору"), lw);
        changed |= ImGui::Checkbox(tr("максимальна (повільніше)##acc"), &s_.accurate_color);
        ...
        changed |= ImGui::Checkbox(tr("0–255 замість 16–235##fr"), &s_.full_range);
        ...
        changed |= ImGui::Checkbox(tr("faststart (для інтернету)##fs"), &s_.faststart);
        label(tr("Захист від збою"), lw);
        changed |= ImGui::Checkbox(tr("MP4/MOV пишеться фрагментами##cs"), &s_.crash_safe);
