...
- **Frames without files on disk.** The game's `startmovie` now writes each frame not into a TGA/JPEG file in a temporary folder but into a Windows named pipe the program opened for that frame number (a FIFO on Linux), so frames go from the game straight into the encoder and nothing about them is written to disk. Frame order, quality and frame rate are the same as before (the fake game gives bit-identical video both ways). When the encoder is busy, the game simply waits on the pipe instead of being paused. The game audio takes the same route and is kept as a WAV in the temporary folder, so crash recovery, parallel rendering and resuming work as before. Nothing is injected into the game, so this works with RTXLauncher too. If a game build does not write into the pipe, the render restarts the game from the same point with files and remembers that for this game (for 30 days or until the game updates). Game page → **Frame transfer**; console: `--frame-transport auto|pipe|files`.
