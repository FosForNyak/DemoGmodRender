...
- **Frames through a pipe instead of files, where the game allows it.** The program offers the game's `startmovie` a Windows named pipe opened for each frame number (a FIFO on Linux) instead of a TGA/JPEG file in a temporary folder, so frames could go straight into the encoder with nothing written to disk; order, quality and frame rate stay the same (the fake game gives bit-identical video both ways), and the game audio takes the same route into the usual WAV. Nothing is injected into the game. The current GMod (regular and RTX) did not write into the pipe in tests — most likely its own path protection ("Attempt to open dangerous file path"), which the program does not bypass: the render then restarts the game from the same point with files, shows the game console lines about it in the log and remembers the result for this game (for 30 days or until the game updates). Game page → **Frame transfer**; console: `--frame-transport auto|pipe|files`.
